...
const Favorites = () => {
  ...
  const fetchFavCars = async () => {
    ...
    try {
      ...
      if (token) {
        ...
        console.log("Using Authorization header for mobile favorites");
      }
  ...
  const handleRemoveFromFav = async (car) => {
    ...
    try {
      ...
      if (token) {
        headers["Authorization"] = `Bearer ${token}`;
        console.log("Using Authorization header for mobile remove favorites");
      }

      const response = await fetch(
        `${config.API_BASE_URL}/favorites/remove/${car.id}/`,
        {
          method: "DELETE",
          credentials: "include",
          headers: headers,
  ...
  const handleAddToCart = async (car) => {
    try {
      ...
      if (token) {
        ...
        console.log("Using Authorization header for mobile add to cart");
      }
      ...
      <div className="favWrapper">
        ...
        <div className="contentFav">
          ...
          <main className="mainContentFav">
            ...
            <p>
              Your favorite cars list is empty. Add some cars to your favorites!
            </p>
          </main>
        </div>
      </div>
        ...
        <main className="mainContentFav">
          {selectedCar ? (
            <div className="car-details">
              ...
              <h1>
                {selectedCar.car_name} ({selectedCar.brand_name})
              </h1>
              <img
                src={
                  selectedCar.photo_url.startsWith("http")
                    ? selectedCar.photo_url
                ...
                {Array.isArray(filteredCars) && filteredCars.length > 0 ? (
                  filteredCars.map((car) => (
                    <div key={car.id} className="fav-car-card">
                      <h3>
                        {car.car_name} ({car.brand_name})
                      </h3>
                      <img
                        src={
                          car.photo_url.startsWith("http")
                            ? car.photo_url
                            : `${config.API_BASE_URL.replace("/api", "")}${
                                car.photo_url
                              }`
